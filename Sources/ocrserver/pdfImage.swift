//
//  pdfImage
//  helps extract images from PDFKit
//
//  From https://gist.github.com/Dev1an/988f2f5fd3b893b2eac2d5539ed4fe3e
...
func extractImages(from pdf: PDFDocument) async -> [EmbeddedImage] {
    ...
    for pageNumber in 0..<pdf.pageCount {
        ...
        let newImages = await extractImages(from: page)
...
func extractImages(from page: PDFPage) async -> [EmbeddedImage] {
    ...
    if let xObject = resources[CGPDFDictionaryGetDictionary, "XObject"] {
        let objects = CGPDFDictionaryGetCount(xObject)
        
        if (objects == 0) {
            return []
        }
        
        let embeddedImages: [EmbeddedImage] = await withCheckedContinuation { continuation in
            var extracted: [EmbeddedImage] = []
            var seen = 0
